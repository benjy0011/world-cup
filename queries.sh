...
echo "$($PSQL "SELECT DISTINCT t.name FROM games g JOIN teams t ON (g.winner_id = t.team_id OR g.opponent_id = t.team_id) WHERE g.round = 'Eighth-Final' AND g.year = 2014 ORDER BY t.name;")"
